...
e = 10**(-17)
def build(): #Building T cross section aprox formula
    
    x = [93.8227*i for i in range(100,100000,10)]
    y = []
    for i in x:
        values_y = 4.42286E+03*(0.519 - 0.0297*i)*(4.25/i)*( (exp(-3.2413/math.sqrt(i)))/(( i - 3.20 )**2 + 6.92) ) 
        y.append(values_y)
    return x, y

def build_table(table_kin_energy, table_cross): #building T cross experemental
...
#     y = []
#     for i in x:
#         values_y = (0.519 - 0.0297*i)*(4.25/i)*( (exp(-3.2413/math.sqrt(i)))/(( i - 3.20 )**2 + 6.92) ) 
#         y.append(values_y)
#     return x, y
...
plot_total_cross_section(
    fig.add_subplot(1, 2, 1),  "building_curves/kozlov_Ekin T.txt", "building_curves/kozlov_cross_section T.txt",
)
plt.show()
